...
class Inspector:
    ...
    def inspectCPUDelta(self):
        if self.__inspectedCPU:
            ...
            for metric in CPUMetrics:
                value = (
                    self.__cpuPolls[totalPolls - 1]["cpuTotal"][metric]
    ...
    def inspectMemoryDelta(self):
        if self.__inspectedMemory:
            ...
            if os.path.isfile("/proc/vmstat"):
                ...
                for line in lines:
                    if "pgfault" in line:
                        self.__attributes["pageFaultsDelta"] = (
                            int(line.split(" ")[1]) - self.__attributes["pageFaults"]
                        )
                    elif "mgmajfault" in line:
                        self.__attributes["majorPageFaultsDelta"] = (
                            int(line.split(" ")[1]) - self.__attributes["majorPageFaults"]
                        )
            else:
                self.__attributes["SAAFMemoryDeltaError"] = "/proc/vmstat does not exist!"
        else:
            self.__attributes["SAAFMemoryDeltaError"] = (
                "Memory not inspected before collecting deltas!"
            )
        ...
        try:
            if self.__inspectedPlatform and self.__inspectedCPUDelta:
                if self.__attributes["platform"] == "AWS Lambda":
                    ...
                    utilizedCPUs = (
                        self.__attributes["cpuUserDelta"] + self.__attributes["cpuKernelDelta"]
                    ) / self.__attributes["userRuntime"]
                ...
                self.__attributes["SAAFRecommendConfigurationError"] = (
                    "CPU, CPU Delta, and Platform must be inspected before recommending a configuration!"
                )
        except Exception as e:
            self.__attributes["SAAFRecommendConfigurationError"] = (
                "Unable to recommend a configuration. " + str(e)
            )
    ...
    def inspectAllDeltas(self):
        ...
        if "frameworkRuntime" in self.__attributes:
            self.addTimeStamp(
                "userRuntime", self.__startTime + self.__attributes["frameworkRuntime"]
            )
